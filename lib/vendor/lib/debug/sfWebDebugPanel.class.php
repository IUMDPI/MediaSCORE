...
abstract class sfWebDebugPanel {
    ...
    public function formatFileLink($file, $line = null, $text = null) {
        ...
        if ($linkFormat = sfConfig::get('sf_file_link_format', ini_get('xdebug.file_link_format'))) {
            // return a link
            return sprintf(
                            '<a href="%s" class="sfWebDebugFileLink" title="%s">%s</a>', htmlspecialchars(strtr($linkFormat, array('%f' => $file, '%l' => $line)), ENT_QUOTES, sfConfig::get('sf_charset')), htmlspecialchars($shortFile, ENT_QUOTES, sfConfig::get('sf_charset')), null === $text ? $shortFile : $text);
        } else if (null === $text) {
