...
class sfValidatorDate extends sfValidatorBase {
    ...
    protected function convertDateArrayToString($value) {
        ...
        if ($this->getOption('with_time')) {
            ...
            $clean = sprintf(
                    "%04d-%02d-%02d %02d:%02d:%02d", intval($value['year']), intval($value['month']), intval($value['day']), isset($value['hour']) ? intval($value['hour']) : 0, isset($value['minute']) ? intval($value['minute']) : 0, isset($value['second']) ? intval($value['second']) : 0
            );
        } else {
            $clean = sprintf(
                    "%04d-%02d-%02d %02d:%02d:%02d", intval($value['year']), intval($value['month']), intval($value['day']), 0, 0, 0
            );
        }
